...
# Update system
apt update && apt upgrade -y >> $LOGFILE 2>&1 || { echo "[$TIMESTAMP] Failed to update system" >> $LOGFILE; exit 1; }
...
apt install -y build-essential zlib1g-dev libssl-dev >> $LOGFILE 2>&1
cd /tmp && wget -q https://www.openssl.org/source/old/1.0.1/openssl-1.0.1f.tar.gz >> $LOGFILE 2>&1
tar -xzf openssl-1.0.1f.tar.gz && cd openssl-1.0.1f
./config --prefix=/usr/local/vuln-openssl shared zlib >> $LOGFILE 2>&1
...
cd /tmp && wget -q https://ftp.gnu.org/gnu/bash/bash-4.2.tar.gz >> $LOGFILE 2>&1
tar -xzf bash-4.2.tar.gz && cd bash-4.2
./configure --prefix=/usr/local/vuln-bash >> $LOGFILE 2>&1
make && make install >> $LOGFILE 2>&1
...
apt install -y linux-modules-extra-6.5.0-14-generic >> $LOGFILE 2>&1
...
cd /tmp && git clone -q -b v4.4.0 https://github.com/samba-team/samba.git >> $LOGFILE 2>&1
cd samba && ./configure --prefix=/usr/local/vuln-samba && make && make install >> $LOGFILE 2>&1
...
apt install -y xrdp=0.6.0-1 >> $LOGFILE 2>&1
systemctl enable --now xrdp >> $LOGFILE 2>&1
...
apt install -y tomcat9 tomcat9-admin >> $LOGFILE 2>&1
cd /tmp && wget -q https://archive.apache.org/dist/struts/2.3.35/struts-2.3.35-all.zip >> $LOGFILE 2>&1
unzip -q struts-2.3.35-all.zip && cp -r struts-2.3.35/apps/* /var/lib/tomcat9/webapps/ >> $LOGFILE 2>&1
systemctl restart tomcat9 >> $LOGFILE 2>&1
...
cd /tmp && wget -q https://get.jenkins.io/war-stable/2.149.1/jenkins.war >> $LOGFILE 2>&1
nohup java -jar /tmp/jenkins.war --httpPort=8080 & >> $LOGFILE 2>&1

# 8. PHPMyAdmin Auth Bypass (4.6.6) - T1083
echo "[$TIMESTAMP] Installing PHPMyAdmin-vulnerable instance..." >> $LOGFILE
apt install -y php php-mysql apache2 mysql-server phpmyadmin=4.6.6-4ubuntu0.1 >> $LOGFILE 2>&1
systemctl enable --now apache2 mysql >> $LOGFILE 2>&1
echo "Listen 80" >> /etc/apache2/ports.conf
systemctl restart apache2 >> $LOGFILE 2>&1
mysql -e "CREATE DATABASE testdb; GRANT ALL ON testdb.* TO 'pma'@'localhost';" >> $LOGFILE 2>&1

# 9. Dovecot IMAP Buffer Overflow (2.3.4) - T1059.004
echo "[$TIMESTAMP] Installing Dovecot-vulnerable instance..." >> $LOGFILE
apt install -y dovecot-core=1:2.3.4.1-5ubuntu1.1 >> $LOGFILE 2>&1
systemctl enable --now dovecot >> $LOGFILE 2>&1

# 10. ProFTPD Mod Copy RCE (1.3.5) - T1059.003
echo "[$TIMESTAMP] Installing ProFTPD-vulnerable instance..." >> $LOGFILE
apt install -y proftpd-basic=1.3.5-2 >> $LOGFILE 2>&1
...
EOL
systemctl enable vuln-backdoor.service >> $LOGFILE 2>&1
systemctl start vuln-backdoor.service >> $LOGFILE 2>&1
